...
\newcommand{\web}{\mbox{$sender_email} $sender_url \par \medskip \normalsize\monthyeardate\today} % Beim Zeilenumbruch \par behalten.
\newcommand{\vspace{20 mm}}
\newcommand{\angbeschr}{\Large Rechnung}
...
\section*{$project_short_description}
$invoice_number \\ $project_description
...
\begin{tabular}{ccp{0.6\textwidth}c}
 ...
 &\multicolumn{3}{r}{{Summe Netto}: {$total_cost \euro}}
  & &\multicolumn{3}{r}{{MwSt. (0\%)}: {$mwst_cost \euro}}
...
\textbf{$project_long_description}
...
\vspace{5 mm}
\begin{center}
\includegraphics[scale=0.35, keepaspectratio=true]{logos/footnote.png}
\end{center}
\end{document}