...
public class BookAdmin extends CustomComponent{
  ...
  public BookAdmin(User user){
        ...
        userRole.setSizeFull();
	  	...
	  	*/
	  	...
	  	updateUser.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				try {
					if(userDevice.getValue().equals(null) || userRole.getValue().equals(null) || userGroup.getValue().equals(null)) {
						showErrorNotification("Something's missing!","Please make sure that you selected a Device, a Role and a Group! Each list has to have one highligthed option.'.");
						...
						DBManager.getDatabaseInstance().getShitDone(
								...
								DBManager.getDatabaseInstance().getUserIDbyLDAPID(LiferayAndVaadinUtils.getUser().getScreenName()),
								DBManager.getDatabaseInstance().getDeviceIDByName(userDevice.getValue().toString())
								);
						
						DBManager.getDatabaseInstance().getShitDoneAgain(
								DBManager.getDatabaseInstance().getUserGroupIDByName(userGroup.getValue().toString()),
	  	...
	  	bookAdmin.addSelectedTabChangeListener(new SelectedTabChangeListener() {
			...
			public void selectedTabChange(SelectedTabChangeEvent event) {
				...
				userRole.select(DBManager.getDatabaseInstance().getUserGroupDescriptionByLDAPId(
						LiferayAndVaadinUtils.getUser().getScreenName(),
						DBManager.getDatabaseInstance().getDeviceIDByName(bookAdmin.getSelectedTab().getCaption())));
				userGroup.select(DBManager.getDatabaseInstance().getUserRoleNameByLDAPId(LiferayAndVaadinUtils.getUser().getScreenName()));
	    ...
	    try
	    {       
	        JDBCConnectionPool connectionPool = new SimpleJDBCConnectionPool("com.mysql.jdbc.Driver", "jdbc:mysql://localhost:8889/facs_facility", "facs", "facs");
	        QueryDelegate qd = new FreeformQuery("select * from facs_facility", connectionPool, "id");
	        final SQLContainer c = new SQLContainer(qd);
	        bookAdmin.setContainerDataSource(c);           
	    }
  ...
  private Component newDeviceGrid(final String deviceName) {
    ...
    gridMap.get(deviceName).getColumn("delete").setRenderer(new ButtonRenderer(new ClickableRenderer.RendererClickListener(){
          @Override
  ...
  private Component deletedBookingsGrid() {
	    ...
	    /*
	    devicesGrid.addColumn("delete", FontIcon.class).setWidth(35)
        ...
        */
	    ...
	    devicesGridTrash.getColumn("delete").setRenderer(new ButtonRenderer(new ClickableRenderer.RendererClickListener(){
	          @Override
	          public void click(RendererClickEvent event) {
	    ...
	    devicesGridTrash.getColumn("restore").setRenderer(new ButtonRenderer(new ClickableRenderer.RendererClickListener(){
	          @Override
	          public void click(RendererClickEvent event) {
  ...
  private Component awaitingRequestsGrid() {
	    ...
	    // Render a button that deletes the data row (item)
	    devicesGridConfirm.getColumn("delete").setRenderer(new ButtonRenderer(new ClickableRenderer.RendererClickListener(){
	          @Override
	          public void click(RendererClickEvent event) {
	    ...
	    devicesGridConfirm.getColumn("confirm").setRenderer(new ButtonRenderer(new ClickableRenderer.RendererClickListener(){
	          @Override
	          public void click(RendererClickEvent event) {
  ...
  private BeanItemContainer<BookingBean> getBookingList(String deviceName) {
	    BeanItemContainer<BookingBean> bookingList = new BeanItemContainer<BookingBean>(BookingBean.class);
	    List<BookingBean> bookings = DBManager.getDatabaseInstance().getAllBookingsPerDevice(deviceName);
	    assert bookings != null;
	    bookingList.addAll(bookings);
  ...
  protected void removeBooking(Grid device,BookingBean db) {
    ...
    if(removed){
      ...
      //TODO log failed operation
    	showErrorNotification("Jeez! It's not fair!","For some reason we couldn't TRASH this booking. Maybe it's already trashed or already purged from the database.");
    }
  }
  ...
  protected void purgeBooking(BookingBean db) {
	    ...
	    if(purged){
	    	...
	    	//TODO log failed operation
	    	showErrorNotification("Jeez! It's not fair!","For some reason we couldn't PURGE this booking. Maybe it's already restored or already purged from the database.");
	    }
	  }
  ...
  protected void restoreBooking(BookingBean db) {
	    ...
	    if(restored){
	    	...
	    	//TODO log failed operation
	    	showErrorNotification("Jeez! It's not fair!","For some reason we couldn't RESTORE this booking. Maybe it's already restored or already removed from the database.");
	    }
	  }
  ...
  protected void denyBooking(BookingBean db) {
	    ...
	    if(denied){
	      ...
	      //System.out.println("Denied");
	    	...
	    	//TODO log failed operation
	    	showErrorNotification("Jeez! It's not fair!","For some reason we couldn't DENY this booking. Maybe it's already denied or already removed from the database.");
	    }
	  }
  ...
  protected void confirmBooking(BookingBean db) {
	    ...
	    if(confirmed){
	    	...
	    	//TODO log failed operation
	     	showErrorNotification("Jeez! It's not fair!","For some reason we couldn't CONFIRM this booking. Maybe it's already confirmed or already removed from the database.");
	    }
	  }
