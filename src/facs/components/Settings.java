...
public class Settings extends CustomComponent{
 ...
 // Create a grid
  ...
  protected void removeDevice(DeviceBean db) {
    ...
    if(removed){
      devicesGrid.getContainerDataSource()
      .removeItem(db);
    }
    else{
  ...
  private void addNewDevice() {
    ...
    name.addValidator(new StringLengthValidator(
        "The name must be 1-85 letters long (Was {0}).",
        1, 85, true));
    ...
    description.addValidator(new StringLengthValidator(
        "The name must be 1-255 letters long (Was {0}).",
        1, 255, true));
    ...
    save.addClickListener(new ClickListener(){
      ...
      public void buttonClick(ClickEvent event) {
        if(name.isValid() && description.isValid()){
          ...
          int deviceId = DBManager.getDatabaseInstance().addDevice(name.getValue(), description.getValue(),(restr.size()==1));
          DeviceBean bean = new DeviceBean(deviceId, name.getValue(),description.getValue(),(restr.size()==1));
    ...
    //add to database
    /*
    boolean added = false;//DBManager.getDatabaseInstance().addDevice(db);
    //TODO test
    //add to grid
    if(added){
      devicesGrid.addRow(db);
    }else{
      //TODO log failed operation
