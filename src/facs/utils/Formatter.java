...
public class Formatter {
  ...
  /**
   * returns a string in the format hours:minutes for a number of milliseconds. For example
   * toHoursAndMinutes(endDate.getTime() - startDate.getTime()); Values below one minute are ignored
   * and truncated.
   * 
  ...
  public static String toHoursAndMinutes(long milliseconds) {
    ...
    float minutes = (milliseconds * milliseconds_to_hours - hours) * hour_in_minutes;
  ...
  public static float toMinutes(long milliseconds) {
    return (milliseconds * milliseconds_to_hours - toHours(milliseconds)) * hour_in_minutes;
  }
